...
Displaying a feed's current value can be helpful in providing an estimated execution price. Because oracle nodes are heavily rate-limited, simulating using a [Crossbar server](../../../../tooling/crossbar/) is incredibly useful during the stages of development.
...
**Why run your own instance?** The public Crossbar node is heavily rate-limited, as are the oracle nodes. In the future, oracle providers may offer API keys that you can plug into Crossbar for elevated rates. But for the moment, if you want to hammer a Crossbar server with simulations, it's recommended that you [run your own instance](../../../../tooling/crossbar/run-crossbar-with-docker-compose.md).
